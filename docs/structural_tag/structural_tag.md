...
- `"gemma"`: Gemma 4 tool-call arguments, JSON-shaped but with bare object keys and strings delimited by `<|"|>` instead of quotes, such as `{key:<|"|>value<|"|>,n:3}`. Keys must be identifiers, so `patternProperties` and `propertyNames` are rejected. String bodies carry no escapes, so a permissive `pattern` or `format` that can generate the delimiter text ends the string early; keep such patterns restrictive.
